...
def plot_confidence_bar(names, means, std_devs, N, title, x_label, y_label, confidence, show=False, filename=None, colors=None, yscale=None):
    ...
    errors = [standard_error(std_devs[i], N[i], confidence)
              for i in range(len(means))]
    fig, ax = plt.subplots()
    x_pos = np.arange(len(names))
    ax.bar(x_pos, means, yerr=errors, align='center', alpha=0.5,
           color=colors if colors is not None else "gray", ecolor='black', capsize=10)
...
def compare_results_learning(results, confidence=0.95, title="Agents Comparison", metric="Steps Per Episode", colors=None):
    ...
    for agent, agent_results in results.items():
        ...
        plt.errorbar(x, y, yerr=yerr, label=agent, ls="dotted", capsize=10, marker="o",
                     color=colors[list(results.keys()).index(agent)] if colors is not None else None)
